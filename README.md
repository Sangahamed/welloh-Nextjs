...
### Supabase CLI — migrations (optional)

Use this if you prefer applying the files under `supabase/migrations/` instead of pasting SQL manually. The **project ref** is the hostname segment in `https://<project-ref>.supabase.co` (Dashboard → Project Settings → General).

```bash
# 1. Initialize local CLI metadata (safe to skip if `supabase/config.toml` already exists)
npx supabase init

# 2. Sign in (browser opens) or use a personal access token for CI
npx supabase login
# npx supabase login --token <your_personal_access_token>

# 3. Link the repo to your hosted project
npx supabase link --project-ref <project-ref>

# 4a. Local database only: start Docker stack, then apply pending migrations to local Postgres
npx supabase start
npx supabase migration up

# 4b. Remote hosted project: push migration files to the linked Supabase project
