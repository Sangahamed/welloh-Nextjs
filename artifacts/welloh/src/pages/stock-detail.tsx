...
export default function StockDetail() {
  ...
  const { data: quote, isLoading } = useGetStockQuote({ ticker, exchange });
  const { data: watchlist } = useGetWatchlist();
  const { mutate: trade, isPending: tradePending } = useExecuteTrade();
  const { mutate: addWatch } = useAddToWatchlist();

  const isWatched = watchlist?.some((w) => w.ticker === ticker && w.exchange === exchange);
  ...
  function handleTrade() {
    ...
    trade(
      { data: { ticker, exchange, quantity: qty, action: tradeType } },
      {
        onSuccess: (result) => {
          toast({
