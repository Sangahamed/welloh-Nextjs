...
import { ChartTrendingUpIcon, GlobeAltIcon, SparklesIcon, ShieldCheckIcon, BookOpenIcon, UsersIcon, TrophyIcon } from '@/components/icons/Icons';
...
// Typewriter effect hook (identique)
...
const FeatureCard: React.FC<{ 
  ...
  return (
    <div 
      ...
      `}
    >
      <NeonCard variant={variant === 'green' ? 'default' : variant} className="p-6 h-full">
        <div className={`
...
// Trust badge component (identique)
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 pt-20">
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          >
            <StatCard value={15000} suffix="+" label={t('activeTraders')} icon={<UsersIcon className="w-6 h-6" />} delay={1600} />
            <StatCard value={2} prefix="$" suffix="M+" label={t('simulatedVolume')} icon={<ChartTrendingUpIcon className="w-6 h-6" />} delay={1800} />
            <StatCard value={50} suffix="+" label={t('globalExchanges')} icon={<GlobeAltIcon className="w-6 h-6" />} delay={2000} />
            <StatCard value={98} suffix="%" label={t('satisfactionRate')} icon={<TrophyIcon className="w-6 h-6" />} delay={2200} />
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white">
              {t('featuresTitle')}
              <span className="block mt-2 text-glow-cyan text-neon-cyan">{t('excel').toLowerCase()} {t('simulate').toLowerCase()}</span>
            </h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FeatureCard
              ...
              title={t('realisticSimulation')}
              description={t('realisticSimulationDesc')}
              variant="cyan"
              delay={0}
            />
            <FeatureCard
              icon={<SparklesIcon className="w-6 h-6" />}
              title={t('advancedAIAnalysis')}
              description={t('advancedAIAnalysisDesc')}
              variant="magenta"
              delay={100}
            />
            <FeatureCard
            ...
            <FeatureCard
              ...
              title={t('competitionsLeaderboards')}
              description={t('competitionsLeaderboardsDesc')}
              variant="green"
              delay={300}
            />
            <FeatureCard
              icon={<BookOpenIcon className="w-6 h-6" />}
              title={t('integratedTraining')}
              description={t('integratedTrainingDesc')}
              variant="cyan"
              delay={400}
            />
            <FeatureCard
              icon={<ShieldCheckIcon className="w-6 h-6" />}
              title={t('maximumSecurity')}
              description={t('maximumSecurityDesc')}
              variant="magenta"
              delay={500}
            />
          </div>
      ...
      <section className="relative py-32 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            {[
              { step: '01', title: t('signUp'), desc: t('signUpDesc') },
              { step: '02', title: t('simulate'), desc: t('simulateDesc') },
              { step: '03', title: t('excel'), desc: t('excelDesc') },
            ].map((item) => (
      ...
      <section className="relative py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white">
              {t('testimonialsTitle')}
              <span className="block mt-2 text-glow-green text-neon-green">{t('traders')}</span>
            </h2>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { name: 'Amadou K.', role: 'Trader Junior - Ecobank', text: t('testimonial1') },
              { name: 'Fatima M.', role: 'Etudiante Finance - HEC Paris', text: t('testimonial2') },
              { name: 'Jean-Pierre L.', role: 'Analyste - SG Afrique', text: t('testimonial3') },
