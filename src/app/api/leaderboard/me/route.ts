...
async function computeLeaderboard(limit: number) {
  ...
  for (const p of portfolios) {
    ...
    for (const sell of sells) {
      ...
      if (buys.length > 0 && parseFloat(sell.price || "0") > buys.reduce((s, b) => s + parseFloat(b.price || "0"), 0) / buys.length) wins++;
    }
    ...
    entries.push({ userId: p.userId, displayName: user?.displayName || `Trader_${p.userId.slice(0, 6)}`, avatarUrl: user?.avatarUrl || null, score: parseFloat(score.toFixed(2)), totalReturn: parseFloat(totalReturn.toFixed(2)), returnPct: parseFloat(returnPct.toFixed(2)), winRate: parseFloat(winRate.toFixed(1)), totalTrades: transactions.length, sharpeRatio });
  }
