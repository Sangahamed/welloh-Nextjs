...
async function ensurePortfolio(userId: string) {
  ...
  if (!portfolio) {
    const [p] = await db.insert(portfoliosTable).values({ userId, name: "Main Portfolio", currentBalance: "100000", initialBalance: "100000" }).returning();
    portfolio = p;
  }
  return portfolio;
}
  ...
  try {
    ...
    if (type === "BUY") {
      ...
      if (existing) {
        ...
        await db.update(holdingsTable).set({ shares: newShares.toFixed(4), averagePrice: avgPrice.toFixed(4) }).where(eq(holdingsTable.id, existing.id));
      } else {
        await db.insert(holdingsTable).values({ portfolioId: portfolio.id, symbol, shares: numShares.toFixed(4), averagePrice: effectivePrice.toFixed(4) });
      }

      const newBalance = currentBalance - total;
      await db.update(portfoliosTable).set({ currentBalance: newBalance.toFixed(4) }).where(eq(portfoliosTable.id, portfolio.id));
      const [transaction] = await db.insert(transactionsTable).values({ portfolioId: portfolio.id, symbol, transactionType: "BUY", shares: numShares.toFixed(4), price: effectivePrice.toFixed(4), commission: commission.toFixed(4), totalValue: total.toFixed(4), status: "completed" }).returning();
      return NextResponse.json({ success: true, message: `Bought ${numShares} shares of ${symbol}`, transaction, newBalance });
      ...
      if (remainingShares < 0.0001) {
        ...
        await db.update(holdingsTable).set({ shares: remainingShares.toFixed(4) }).where(eq(holdingsTable.id, existing.id));
      }

      const proceeds = grossTotal - commission;
      const newBalance = currentBalance + proceeds;
      await db.update(portfoliosTable).set({ currentBalance: newBalance.toFixed(4) }).where(eq(portfoliosTable.id, portfolio.id));
      const [transaction] = await db.insert(transactionsTable).values({ portfolioId: portfolio.id, symbol, transactionType: "SELL", shares: numShares.toFixed(4), price: effectivePrice.toFixed(4), commission: commission.toFixed(4), totalValue: proceeds.toFixed(4), status: "completed" }).returning();
      return NextResponse.json({ success: true, message: `Sold ${numShares} shares of ${symbol}`, transaction, newBalance });
