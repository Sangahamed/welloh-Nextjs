...
export async function GET() {
  try {
    ...
    const holdingsWithPrices = holdings.map((h: any) => {
      ...
      return { id: h.id, symbol: h.symbol, shares, averagePrice, currentPrice, currentValue: parseFloat(currentValue.toFixed(2)), gainLoss: parseFloat(gainLoss.toFixed(2)), gainLossPct: parseFloat(gainLossPct.toFixed(2)) };
    });

    return NextResponse.json({ id: portfolio.id, name: portfolio.name, currentBalance: parseFloat(portfolio.currentBalance || "0"), initialBalance: parseFloat(portfolio.initialBalance || "0"), holdings: holdingsWithPrices });
  } catch (err) {
