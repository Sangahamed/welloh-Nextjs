...
export const STOCK_DATABASE: StockInfo[] = [
  ...
  },
  {
    ticker: "ETIT",
    exchange: "BRVM",
    companyName: "Ecobank Transnational Inc.",
    sector: "Financial Services",
    currency: "XOF",
    basePrice: 14.5,
    region: "Africa",
  },
  {
    ticker: "BICI",
    exchange: "BRVM",
    companyName: "BICICI - Banque Internationale",
    sector: "Financial Services",
    currency: "XOF",
    basePrice: 7600,
    region: "Africa",
  ...
  },
  {
    ticker: "GTCO",
    exchange: "NSE",
    companyName: "Guaranty Trust Holding Co.",
    sector: "Financial Services",
    currency: "NGN",
    basePrice: 42.3,
    region: "Africa",
  ...
  },
  {
    ticker: "HRHO",
    exchange: "EGX",
    companyName: "Herfy Food Services",
...
function getSimulatedPrice(
  ...
  const seed =
    ticker.charCodeAt(0) * 137 + ticker.charCodeAt(1) * 37 + daysAgo * 17;
...
export function generateStockQuote(stock: StockInfo) {
  ...
  const price = parseFloat(
    (baseNow * (0.995 + seededRandom(today / dayMs) * 0.01)).toFixed(4),
  );
  ...
  const high = parseFloat(
    (price * (1 + seededRandom(today / dayMs + 1) * 0.02)).toFixed(4),
  );
  const low = parseFloat(
    (price * (1 - seededRandom(today / dayMs + 2) * 0.02)).toFixed(4),
  );
  const open = parseFloat(
    (
      previousClose *
      (1 + (seededRandom(today / dayMs + 3) - 0.5) * 0.01)
    ).toFixed(4),
  );
  ...
  for (let i = 30; i >= 0; i--) {
    ...
    historicalData.push({
      ...
      volume: Math.floor(
        seededRandom(i * 31 + stock.ticker.charCodeAt(0)) * 5000000 + 100000,
      ),
    });
  }
...
export function getMarketOverview() {
  ...
  return MARKET_INDICES.map((idx) => {
    ...
    const changePct = parseFloat(
      ((seededRandom(seed) - 0.47) * 2.5).toFixed(2),
    );
    const value = parseFloat(
      (idx.baseValue * (1 + changePct / 100)).toFixed(2),
    );
...
export function findStock(
  ...
  return STOCK_DATABASE.find(
    (s) => s.ticker === ticker && s.exchange === exchange,
  );
}
